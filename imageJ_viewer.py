    ...
    def send_image(self,image):
        ...
        if self.image_dimensions_sent == False : 
            self.pv_rec['dimension'] = [
                {'size': image.shape[1], 'fullSize': image.shape[1], 'binning': 1},
        ...
        if dataType == np.uint16:
            self.EPICS_dtype = "ushortValue"
        if dataType == np.float32 : 
            self.EPICS_dtype = "floatValue"


        #send data to ImageJ viewer
        self.pv_rec['value'] = ({self.EPICS_dtype: image.flatten()},)
